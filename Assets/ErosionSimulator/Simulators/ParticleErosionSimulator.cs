...
namespace ErosionSimulator
{
    class Drop
    {
        ...
        public Drop(Vector2D position, double water)
        {
            this.position = position;
            this.water = water;
            velocity = 0d;
            sediment = 0d;
            direction = new Vector2D(0d, 0d);
        }
    }

    public class ParticleErosionSimulator : Simulator
    {
        ...
        public ParticleErosionSimulator(int sizeX, int sizeY, TerrainProvider terrainProvider)
        {
            this.SizeX = sizeX;
            this.SizeY = sizeY;
        ...
        {
            Drop drop = new Drop(new Vector2D(random.NextDouble() * SizeX, random.NextDouble() * SizeY), DROP_SIZE);
            ...
            for (int i = 0; i < DROP_ITERATIONS; i++)
            {
                ...
                Vector2D directionNew = drop.direction * INERTIA - GetGradient(drop.position) * (1 - INERTIA);

                if (directionNew.IsZero())
                {
                    // pick a random direction
                    double angle = 2 * Math.PI * random.NextDouble();
                    directionNew = new Vector2D(Math.Cos(angle), Math.Sin(angle));
                }
        ...
        private Vector2D GetGradient(Vector2D position)
        {
            ...
            return new Vector2D(InterpolateLinear(terrain[highX, lowY] - terrain[lowX, lowY], terrain[highX, highY] - terrain[lowX, highY], yModulo),
                InterpolateLinear(terrain[lowX, highY] - terrain[lowX, lowY], terrain[highX, highY] - terrain[highX, lowY], xModulo));
        }

        private void DepositSediment(Vector2D position, double sediment)
        {
            int lowX = (int)Math.Floor(position.x);
            int highX = (int)Math.Ceiling(position.x);
            int lowY = (int)Math.Floor(position.y);
        ...
        private void ErodeSediment(Vector2D position, double sediment)
        {
