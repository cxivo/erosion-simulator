...
{
    public class SecondPipeModelSimulator : Simulator
    {
        private const double INITIAL_RAIN = 0.5d;  // meters
        private const double INITIAL_SEDIMENT = 0d;  // meters
        private const double RAIN_HEIGHT = 0.5d;  // meters
        ...
        private const double SEDIMENT_CAPACITY_CONSTANT = 0.02d;
        ...
        private Vector3D[,] previousFlowVelocity;
        ...
        public SecondPipeModelSimulator(int sizeX, int sizeY, TerrainProvider terrainProvider)
        {
            this.SizeX = sizeX;
            this.SizeY = sizeY;
            ...
            this.previousFlowVelocity = new Vector3D[SizeX, SizeY];
            ...
            originalHeight = terrain[SizeX - 1, SizeY / 2];
        }
                ...
                for (int y = 0; y < SizeY; y++)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        if (isInBounds(x + deltaX[i], y + deltaY[i]))
                        {
                            ...
                            water[x, y].Sediment -= 0.2d * originalSediment;
                        }
                    }
        ...
        private void ManageRiver()
        {
            ...
            for (int y = -20; y < 20; y++)
            {
                water[SizeX - 1, y + SizeY / 2].Height = originalHeight - terrain[SizeX - 1, y + SizeY / 2] + 0.3d;                
                water[SizeX - 1, y + SizeY / 2].Sediment = 0d;                
            }
            ...
            for (int x = 0; x < SizeX; x++)
            {
                for (int y = 0; y < SizeY; y++)
                {
                    if (water[x, y].Height > 0d)
                    {
                        ...
                        water[x, y].Velocity = flowVelocity / (CELL_SIZE * 0.5d * (water[x, y].Height + water[x, y].PreviousHeight + 0.3d));
                        ...
                        double sedimentTransportCapacity = water[x, y].Velocity.Magnitude() * SEDIMENT_CAPACITY_CONSTANT * SteepnessAt(x, y);
                        ...
                        if (water[x, y].Sediment <= sedimentTransportCapacity)
                        {
                            ...
                            //water2[x, y] += new WaterColumn(sedimentHeight, sedimentHeight);
                        ...
                        else
                        {
                            ...
                            //water2[x, y] -= new WaterColumn(sedimentHeight, sedimentHeight, -water2[x, y].Velocity);
                            water[x, y].Sediment -= sedimentHeight;
                        }

                        previousFlowVelocity[x, y] = flowVelocity;
                        ...
                        for (int i = 0; i < 4; i++)
                        {
                            if (isInBounds(x + deltaX[i], y + deltaY[i]))
                            {
                                ...
                                double regolithMovement = 0.2d * Math.Max(0d, DELTA_T * GRAVITY * deltaHeight);
                                if (Math.Abs(regolithMovement) > regolithDepth)
                                {
                                    terrain[x, y] -= regolithDepth * DELTA_T;
                                    terrain[x + deltaX[i], y + deltaY[i]] += regolithDepth * DELTA_T;
                                }
                            }
                        }
        ...
        private double SteepnessAt(int x, int y)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if (isInBounds(x + deltaX[i], y + deltaY[i]))
                {
                    double deltaHeight = terrain[x + deltaX[i], y + deltaY[i]] - terrain[x, y];
                    double steepness = Math.Abs(deltaHeight) / Math.Sqrt(CELL_SIZE * CELL_SIZE + deltaHeight * deltaHeight);
