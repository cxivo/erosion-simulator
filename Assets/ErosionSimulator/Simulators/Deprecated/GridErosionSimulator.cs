                            ...
                            for (int j = -1; j <= 1; j++)
                            {
                                if (!(i == 0 && j == 0))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        neighbors[1 + i, 1 + j] = Math.Exp(-WATER_SPREAD * (new Vector3D(i, j, 0d) - water[x, y].Velocity.NormalizedOrSmaller()).SquareMagnitude());
                                ...
                                for (int j = -1; j <= 1; j++)
                                {
                                    if (!(i == 0 && j == 0))
                                    {
                                        if (!(x + i < 0 || y + j < 0 || x + i > SizeX - 1 || y + j > SizeY - 1))
                                        {
                                            ...
                                            Vector3D partVelocity = water[x, y].Velocity
                                                + DELTA_T * (new Vector3D(i, j, 0d) * heightDifferences[1 + i, 1 + j] * GRAVITY)
