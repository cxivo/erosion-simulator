...
{
    public class GridBasedSimulator : Simulator
    {
        ...
        private double sedimentCapacity = 0.1d;
        private double collisionErosion = 0.5d;
        private const double MAXIMUM_REGOLITH_THICKNESS = 0.05d;  // meters
        ...
        private TerrainProcessingModule[] modules;

        private int sinceLastEvaporation = 0;
        private int sinceLastDiffusion = 0;
        private double originalHeight;

        // each of the modules is responsible for some type of erosion
        // they get called at each simulation step to simulate their part
        // this way there is more customizablity in how the simulator operates
        public abstract class TerrainProcessingModule
        {
            public abstract void Process(GridBasedSimulator simulator);
        }

        // erodes from the tile the water is in according to the water velocity and terrain steepness
        public class ForceErosionModule : TerrainProcessingModule
        {
            public override void Process(GridBasedSimulator simulator)
            {
                for (int x = 0; x < simulator.SizeX; x++)
                {
                    for (int y = 0; y < simulator.SizeY; y++)
                    {
                        if (simulator.water[x, y].Height > 0d)
                        {
                            // velocity is the average of the inflows and outflows                        
                            Vector3D flowVelocity = GetFlowVelocityAt(simulator, x, y);

                            // FORCE-BASED EROSION
                            // scale by the water height
                            double averageHeight = 0.5d * (simulator.water[x, y].Height + simulator.water[x, y].PreviousHeight);
                            ...
                            averageHeight = Math.Max(averageHeight, 0.2d);
                            ...
                            double sedimentTransportCapacity = velocity.Magnitude() * simulator.sedimentCapacity * simulator.SteepnessAt(x, y);

                            if (simulator.water[x, y].Sediment <= sedimentTransportCapacity)
                            {
                                // erode
                                // how much sediment to erode
                                double sedimentHeight = (sedimentTransportCapacity - simulator.water[x, y].Sediment) * EROSION_FRACTION;

                                // erode from terrain
                                simulator.terrain[x, y] -= sedimentHeight;
                                ...
                                simulator.water[x, y].Sediment += sedimentHeight;
                            }
                            else
                            {
                                // deposit
                                // how much sediment to deposit
                                double sedimentHeight = (simulator.water[x, y].Sediment - sedimentTransportCapacity) * DEPOSIT_FRACTION;
        ...
        // erode from the neighboring cells using height differences
        public class CollisionErosionModule : TerrainProcessingModule
        {
            public override void Process(GridBasedSimulator simulator)
            {
                for (int x = 0; x < simulator.SizeX; x++)
                {
                    for (int y = 0; y < simulator.SizeY; y++)
                    {
                        if (simulator.water[x, y].Height > 0d)
                        {
                            ...
                            for (int i = 0; i < 4; i++)
                            {
                                if (simulator.isInBounds(x + deltaX[i], y + deltaY[i]))
                                {
                                    // how much terrain height is our water column adjacent to
                                    double erodeableHeight = Math.Max(0d, simulator.terrain[x + deltaX[i], y + deltaY[i]] - simulator.terrain[x, y]);

                                    // sine
                                    double slope = erodeableHeight / Math.Sqrt(erodeableHeight * erodeableHeight + CELL_SIZE * CELL_SIZE);

                                    double flow = Math.Max(0d, flowVelocity.x * deltaX[i] + flowVelocity.y * deltaY[i]);

                                    double sedimentHeight = Math.Min((flow / (CELL_SIZE * CELL_SIZE)) * slope * DELTA_T * simulator.collisionErosion, erodeableHeight);

                                    // erode from terrain
                                    simulator.terrain[x + deltaX[i], y + deltaY[i]] -= sedimentHeight;
        ...
        // erode from the neighboring cells using angles of the water stream and the terrain
        public class VectorCollisionErosionModule : TerrainProcessingModule
        {
            public override void Process(GridBasedSimulator simulator)
            {
                for (int x = 0; x < simulator.SizeX; x++)
                {
                    for (int y = 0; y < simulator.SizeY; y++)
                    {
                        if (simulator.water[x, y].Height > 0d)
                        {
                            ...
                            double neighborHeight = simulator.GetHeightAt(x - normalizedFlowVelocity.x, y - normalizedFlowVelocity.y);
                            if (!double.IsNegativeInfinity(neighborHeight))
                            {
                                normalizedFlowVelocity.z = (simulator.terrain[x, y] - neighborHeight) * 0.2d;
                                normalizedFlowVelocity = normalizedFlowVelocity.Normalized();

                                simulator.water[x, y].Velocity = Vector3D.InterpolateLinear(normalizedFlowVelocity, simulator.water[x, y].Velocity, INERTIA).Normalized();
                                ...
                                for (int i = 0; i < 4; i++)
                                {
                                    if (simulator.isInBounds(x + deltaX[i], y + deltaY[i]))
                                    {
                                        Vector3D terrainNormal = new Vector3D(
                                            deltaX[i] * (simulator.terrain[x, y] - simulator.terrain[x + deltaX[i], y + deltaY[i]]) / CELL_SIZE,
                                            deltaY[i] * (simulator.terrain[x, y] - simulator.terrain[x + deltaX[i], y + deltaY[i]]) / CELL_SIZE,
                                            1d).Normalized();

                                        // scalar product
                                        double force = Math.Max(0d, -(terrainNormal * simulator.water[x, y].Velocity));
                                        if (force > 0d)
                                        {
                                            double erodeableHeight = Math.Min(force * CELL_SIZE, simulator.water[x, y].Height);

                                            double flow = Math.Max(0d, simulator.water[x, y].Velocity.x * deltaX[i] + simulator.water[x, y].Velocity.y * deltaY[i]) * erodeableHeight / simulator.water[x, y].Height;

                                            double sedimentHeight = Math.Min((flow / (CELL_SIZE * CELL_SIZE)) * force * DELTA_T * simulator.collisionErosion, erodeableHeight);

                                            // erode from simulator.terrain
                                            simulator.terrain[x + deltaX[i], y + deltaY[i]] -= sedimentHeight;

                                            // add a simulator.water column made entirely out of sediment to the existing column
                                            // this way, the height of simulator.terrain + simulator.water will remain constant + the simulator.water gets slowed down a bit
                                            //simulator.water[x, y] += new WaterColumn(sedimentHeight, sedimentHeight);
                                            simulator.water[x, y].Sediment += sedimentHeight;
                                        }
                                    }
                                }
        ...
        // erode by directly moving sediment to/from neighboring cells, simulating river and lake bed erosion
        public class DissolutionErosionModule : TerrainProcessingModule
        {
            public override void Process(GridBasedSimulator simulator)
            {
                for (int x = 0; x < simulator.SizeX; x++)
                {
                    for (int y = 0; y < simulator.SizeY; y++)
                    {
                        if (simulator.water[x, y].Height > 0d)
                        {
                            ...
                            double regolithDepth = Math.Min(Math.Max(0d, 0.025d * (simulator.water[x, y].Height)), MAXIMUM_REGOLITH_THICKNESS);
                            ...
                            for (int i = 0; i < 4; i++)
                            {
                                if (simulator.isInBounds(x + deltaX[i], y + deltaY[i]))
                                {
                                    ...
                                    double deltaHeight = simulator.terrain[x, y] - simulator.terrain[x + deltaX[i], y + deltaY[i]];
                                    ...
                                    simulator.terrain[x, y] -= Math.Abs(regolithMovement) * DELTA_T;
                                    simulator.terrain[x + deltaX[i], y + deltaY[i]] += Math.Abs(regolithMovement) * DELTA_T;
                                }
                            }
                        }
                    }
        ...
        public GridBasedSimulator(int sizeX, int sizeY, TerrainProvider terrainProvider)
        {
            this.SizeX = sizeX;
            this.SizeY = sizeY;
            ...
            this.modules = new TerrainProcessingModule[] {
                new ForceErosionModule(),
                new CollisionErosionModule(),
                new DissolutionErosionModule()
            };
        ...
        public GridBasedSimulator(int sizeX, int sizeY, TerrainProvider terrainProvider, TerrainProcessingModule[] modules) : this(sizeX, sizeY, terrainProvider)
        {
        ...
        // spreads the sediment of the water columns to prevent instabilities
        ...
        public override void Step()
        {
            ...
            // call each erosion module
            foreach (TerrainProcessingModule module in modules)
            {
                module.Process(this);
            }
        ...
        public override void AddWater(double height)
        {
            for (int i = 0; i < SizeX; i++)
            {
                for (int j = 0; j < SizeY; j++)
                {
                    water[i, j].Height += height;
                }
            }
        }

        public override void RemoveWater(double height)
        {
            for (int x = 0; x < SizeX; x++)
            {
                for (int y = 0; y < SizeY; y++)
                {
                    if (water[x, y].Height > height)
                    {
                        water[x, y].Height -= height;
                        //evaporatedAmount += height;
                    }
                    else
                    {
                        //evaporatedAmount += water[x, y].Height;
                        terrain[x, y] += water[x, y].Sediment;
                        water[x, y] = new WaterColumn();
                    }
                }
            }
        }

        public override void SetErosionCoefficient(double value, double forceValue)
        {
            sedimentCapacity = value;
            collisionErosion = forceValue;
        }

        public override void SetWaterSource(bool state)
        {
            waterSource = state;
        }

        // velocity is the average of the inflows and outflows   
        private static Vector3D GetFlowVelocityAt(GridBasedSimulator simulator, int x, int y)
        {                                
            return new Vector3D(
                0.5d * ((x > 1 ? (simulator.outFlow[x - 1, y, 0] - simulator.outFlow[x, y, 2]) : 0d) + (x < simulator.SizeX - 1 ? (simulator.outFlow[x, y, 0] - simulator.outFlow[x + 1, y, 2]) : 0d)),
                0.5d * ((y > 1 ? (simulator.outFlow[x, y - 1, 1] - simulator.outFlow[x, y, 3]) : 0d) + (y < simulator.SizeY - 1 ? (simulator.outFlow[x, y, 1] - simulator.outFlow[x, y + 1, 3]) : 0d)),
                0d);
        }
        ...
        // gets the steepness and convexness of a particular point of the terrain
        ...
        public override double GetWaterHeightAt(double x, double y)
        {
            ...
            if (result == 0d)
            {
                ...
                else
                {
                    ...
                    return base.GetWaterHeightAt(x, y);
                }
            }
